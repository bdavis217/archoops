...
const Admin: React.FC = () => {
  ...
  const processCompletedMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Use setTimeout to prevent race conditions during navigation
      setTimeout(() => {
        // More specific invalidations to prevent broad query refetching
        queryClient.invalidateQueries({ queryKey: ['games', 'upcoming'] });
        queryClient.invalidateQueries({ queryKey: ['games', 'completed'] });
        queryClient.invalidateQueries({ queryKey: ['predictions', 'my'] });
        queryClient.invalidateQueries({ queryKey: ['predictions', 'leaderboard'] });
